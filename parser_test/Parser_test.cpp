...
CParser_test::CParser_test (void)
	:mp_top (NULL)
	,mp_current (NULL)
	,mp_bottom (NULL)
	,m_buff_size (0)
...
void CParser_test::run (uint8_t *pBuff, size_t size)
{
	if ((!pBuff) || (size == 0)) {
		return ;
	}
...
bool CParser_test::allocInnerBuffer (uint8_t *pBuff, size_t size)
{
	if ((!pBuff) || (size == 0)) {
		return false;
	}
...
// 静的領域にコピー
bool CParser_test::copyInnerBuffer (uint8_t *pBuff, size_t size)
{
	if ((!pBuff) || (size == 0)) {
		return false;
	}
...
bool CParser_test::checkUnitSize (void)
{
	int i;
	int m;
	int n;
...
uint8_t * CParser_test::getSyncTopAddr (uint8_t *pTop, uint8_t *p_btm, size_t unit_size) const
{
...
/*
void CParser_test::getTsHeader (TS_HEADER *pDst, uint8_t* pSrc) const
{
	if ((!pSrc) || (!pDst)) {
		return;
	}
...
void CParser_test::dumpTsHeader (const TS_HEADER *p) const
{
	if (!p) {
		return ;
	}
