	...
	,m_is_enable_EIT_sched (false)
{
	...
	threadmgr::sequence_t seqs [_max] = {
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_module_up(p_if);}, std::move("on_module_up")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_module_down(p_if);}, std::move("on_module_down")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_get_state(p_if);}, std::move("on_get_state")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_check_loop(p_if);}, std::move("on_check_loop")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_parser_notice(p_if);}, std::move("on_parser_notice")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_stabilization_after_tuning(p_if);}, std::move("on_stabilization_after_tuning")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_register_pat_detect_notify(p_if);}, std::move("on_register_pat_detect_notify")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_unregister_pat_detect_notify(p_if);}, std::move("on_unregister_pat_detect_notify")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_register_event_change_notify(p_if);}, std::move("on_register_event_change_notify")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_unregister_event_change_notify(p_if);}, std::move("on_unregister_event_change_notify")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_register_psisi_state_notify(p_if);}, std::move("on_register_psisi_state_notify")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_unregister_psisi_state_notify(p_if);}, std::move("on_unregister_psisi_state_notify")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_get_pat_detect_state(p_if);}, std::move("on_get_pat_detect_state")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_get_stream_infos(p_if);}, std::move("on_get_stream_infos")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_get_current_service_infos(p_if);}, std::move("on_get_current_service_infos")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_get_present_event_info(p_if);}, std::move("on_get_present_event_info")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_get_follow_event_info(p_if);}, std::move("on_get_follow_event_info")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_get_current_network_info(p_if);}, std::move("on_get_current_network_info")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_enable_parse_eit_sched(p_if);}, std::move("on_enable_parse_eit_sched")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_disable_parse_eit_sched(p_if);}, std::move("on_disable_parse_eit_sched")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_dump_caches(p_if);}, std::move("on_dump_caches")},
		{[&](threadmgr::CThreadMgrIf *p_if){CPsisiManager::on_dump_tables(p_if);}, std::move("on_dump_tables")},
	};
...
bool CPsisiManager::on_ts_packet_available (TS_HEADER *p_header, uint8_t *p_payload, size_t payload_size)
{
	if (!p_header || !p_payload || payload_size == 0) {
		// through
		return true;
	}
	...
	case PID_PAT: {
		...
		_parser_notice_t _notice (CPsisiManagerIf::psisi_type::PAT, p_header, p_payload, payload_size);
		r = request_async (
	...
	case PID_EIT_H: {
		...
		_parser_notice_t _notice (CPsisiManagerIf::psisi_type::EIT_H_PF, p_header, p_payload, payload_size);
		r = request_async (
			static_cast<uint8_t>(modules::module_id::psisi_manager) + getGroupId(),
			static_cast<int>(CPsisiManagerIf::sequence::parser_notice),
			(uint8_t*)&_notice,
	...
	case PID_NIT: {
		...
		_parser_notice_t _notice (CPsisiManagerIf::psisi_type::NIT, p_header, p_payload, payload_size);
		r = request_async (
			static_cast<uint8_t>(modules::module_id::psisi_manager) + getGroupId(),
			static_cast<int>(CPsisiManagerIf::sequence::parser_notice),
			(uint8_t*)&_notice,
	...
	case PID_SDT: {
		...
		_parser_notice_t _notice (CPsisiManagerIf::psisi_type::SDT, p_header, p_payload, payload_size);
		r = request_async (
			static_cast<uint8_t>(modules::module_id::psisi_manager) + getGroupId(),
			static_cast<int>(CPsisiManagerIf::sequence::parser_notice),
			(uint8_t*)&_notice,
	...
	case PID_CAT: {
		...
		_parser_notice_t _notice (CPsisiManagerIf::psisi_type::CAT, p_header, p_payload, payload_size);
		r = request_async (
			static_cast<uint8_t>(modules::module_id::psisi_manager) + getGroupId(),
			static_cast<int>(CPsisiManagerIf::sequence::parser_notice),
			(uint8_t*)&_notice,
	...
	case PID_CDT: {
		...
		_parser_notice_t _notice (CPsisiManagerIf::psisi_type::CDT, p_header, p_payload, payload_size);
		r = request_async (
			static_cast<uint8_t>(modules::module_id::psisi_manager) + getGroupId(),
			static_cast<int>(CPsisiManagerIf::sequence::parser_notice),
			(uint8_t*)&_notice,
...
//	case PID_RST:
//		r = mRST.checkSection (p_header, p_payload, payload_size);
//		break;

//	case PID_BIT:
//		r = mBIT.checkSection (p_header, p_payload, payload_size);
//		break;

	default:
		// parse PMT
		if (m_program_map.has(p_header->pid)) {
			_parser_notice_t _notice (CPsisiManagerIf::psisi_type::PMT, p_header, p_payload, payload_size);
			r = request_async (
				static_cast<uint8_t>(modules::module_id::psisi_manager) + getGroupId(),
				static_cast<int>(CPsisiManagerIf::sequence::parser_notice),
				(uint8_t*)&_notice,
