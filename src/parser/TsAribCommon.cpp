...
static const uint8_t g_commonPaletCLUT [] = {
	// length(4bytes), chunk type (PLTE)
	0x00, 0x00, 0x01, 0x80, 0x50, 0x4C, 0x54, 0x45,
	// 以下CLUT共通固定色 64色 alpha入りも同じように並べたもの
...
uint32_t CTsAribCommon::crc32(const uint8_t *start, size_t length)
{
...
const uint8_t *CTsAribCommon::getCommonPaletCLUT (void)
{
	return &g_commonPaletCLUT[0];
}

size_t CTsAribCommon::getCommonPaletCLUTSize (void)
{
	return sizeof(g_commonPaletCLUT);
}
